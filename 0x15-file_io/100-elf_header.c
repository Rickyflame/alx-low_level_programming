 ...
 * Return: returns 0 if first n bytes of s1 and s2
 * are equal else 0
 */
...
/**
 * _close - closes a file descriptor and prints an error
 * message on failure
...
/**
 * _read - reads from a file and prints an erro message
 * on failure
 * @fd: the filedescriptor toread from
 * @buf: the buffer to write to
 * @count: number of bytes to read
 * Return: void has no retunrn value
 */
...
/**
 * elf_magic - printf the ELF magic
...
void elf_magic(const unsigned char *buffer)
{
	...
	{
		write(STDERR_FILENO, "Error: Not an RLF file\n", 23);
		exit(98);
	}
	printf("ELF Header:\n Magic:	");
...
/**
 * elf_class - printfELF class
 * @buffer: theELF header
 * Return: bit mode (32 0r 64)
 */
size_t elf_class(const unsigned char *buffer)
{
	printf("  %-34s ", "Class:");

	if (buffer[EI_CLASS] == ELCLASS64)
	{
		printf("ELF64\n");
		return (64);
	}
...
/**
 * elf_abivers - printf ELF ABI version
 * @buffer: the ELF header
 * Return: void has no return value
 */
